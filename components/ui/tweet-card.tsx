...
export const TweetMedia = ({ tweet }: { tweet: EnrichedTweet }) => {
  ...
  return (
    <div className="flex flex-1 items-start justify-start gap-4 overflow-x-auto">
      {tweet.video && (
        <video
      ...
      {tweet.photos &&
        tweet.photos.map((photo) => {
          const url = photo.url.replace(/name=\w+$/, "name=orig");
          return (
            <img
              key={photo.url}
              src={url}
              ...
              alt={tweet.text}
              className="h-96 w-auto rounded-xl border object-cover shadow-sm"
            />
          );
        })}
    </div>
